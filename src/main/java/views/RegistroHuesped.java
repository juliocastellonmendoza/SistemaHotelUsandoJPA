	...
	public RegistroHuesped() {
		...
		btnguardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				insertar();
				limpiar();
				txtNreserva.setText(String.valueOf(rs.getIdreserva()));
			}
		});
...
//hecho con JPA
	public void insertar() {
		...
		if(txtApellido.getText().length()==0) {
			JOptionPane.showMessageDialog(rootPane, "Debes ingresar el apellido del huesped");
			txtApellido.requestFocus();
			return;
		}

		if(txtTelefono.getText().length()==0) {
			JOptionPane.showMessageDialog(rootPane, "Debes ingresar el teléfono del huesped");
			txtTelefono.requestFocus();
			return;
		}
		
		if(txtFechaN.getDate() == null) {
			JOptionPane.showMessageDialog(rootPane, "Debes ingresar fecha de nacimiento");
			txtFechaN.requestFocus();
			return;
		}
		
		// Para pasar las fechas de entra y salida de Jcalendar a LocalDate
		SimpleDateFormat sdfi = new SimpleDateFormat("yyyy/MM/dd");
		String fi = null;
		LocalDate fechi = null;
		fi = sdfi.format(ReservasView.txtFechaEntrada.getDate());
		fechi = LocalDate.parse(fi, DateTimeFormatter.ofPattern("yyyy/MM/dd"));
		
		
		SimpleDateFormat sdff = new SimpleDateFormat("yyyy/MM/dd");
		String ff = null;
		LocalDate fechf = null;
		ff = sdff.format(ReservasView.txtFechaSalida.getDate());
		fechf = LocalDate.parse(ff, DateTimeFormatter.ofPattern("yyyy/MM/dd"));

		double valor =Double.parseDouble(ReservasView.txtValor.getText());
		
		int seleccionadoo = ReservasView.txtFormaPago.getSelectedIndex();
		String forma_pago = (String)ReservasView.txtFormaPago.getItemAt(seleccionadoo);
		
		//Instancio la clase reserva para pasarle los datos obtenidos del formulario
		Reservas reserva = new Reservas(fechi, fechf, valor, forma_pago);
		
		//Obtengo los datos del formulario para el huesped
		String nombre = txtNombre.getText();
		String apellido = txtApellido.getText();
		
		// Otra forma de pasar las fechas de Jcalendar a LocalDate para la fecha de nacimiento
		SimpleDateFormat sdfn = new SimpleDateFormat("yyyy/MM/dd");
		String fnaci = null;
		LocalDate fn = null;
		fnaci = sdfn.format(txtFechaN.getDate());
		fn = LocalDate.parse(fnaci, DateTimeFormatter.ofPattern("yyyy/MM/dd"));
		
		int seleccionado = txtNacionalidad.getSelectedIndex();
		String nacionalidad = (String)txtNacionalidad.getItemAt(seleccionado);
		...
		huespedes.agregarItems(nombre, apellido, fn, nacionalidad, telefono);
		...
		JOptionPane.showMessageDialog(rootPane, "El huesped fue registrado satifactoriamente");
